#import "common.typ": *
// NOTE: upstream also has `#import "cover.typ": make_cover`. That module is
// dropped here: it draws an ODE phase portrait over an IBLODEs background
// image, and its trailing self-preview call runs at import time.
#import "settings.typ": global_settings
#import "environments.typ": definition, example, mono, sans, serif, theorem
#import "environment-module.typ": module, module_settings
#import "environment-slides.typ": slide, slide_settings
#import "environment-question.typ": label_question_part, question, question_settings
#import "environment-solutions.typ": solution
#import "environment-only.typ": book_only, slides_only
#import "environment-practice-problems.typ": practice_problem_solutions, practice_problems
#import "environment-guide.typ": learning_objectives, lesson, lessons_end, notes
#import "utils.typ": aligned_terms, simple_table
#import "allow-missing-refs.typ": allow_missing_refs
#import "graphics/graphics.typ": draw_vec, slope_field, vector_field
#import "graphics/graphics3d.typ": slope_field_3d
#import "graphics/spreadsheet.typ": draw_spreadsheet
#import "numerics/ode_solvers.typ": solve_1d_ivp, solve_2d_ivp
