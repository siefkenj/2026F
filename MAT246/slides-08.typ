...
#exercise(
  ...
  2. Use induction to prove that $phi_n$, $psi_n$ are well-defined injective functions for each
    $n in NN$.
...
#exercise(
  ...
  2. Prove that $f(X_"no") = Y_"no"$, and that $f': X_"no" -> Y_"no"$ given by $f'(x) = f(x)$ is a
    bijection.
